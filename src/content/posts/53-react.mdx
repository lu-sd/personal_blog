...
* alert uses newName: The alert uses the newName directly, ensuring it reflects the value just entered by the user.
 ...
 If your only goal is to show the name immediately after the user inputs it, and you don't need to store the name for later use in the component, then you don't need to use React state (useState) at all.
 ```jsx
 export default function FeedbackForm() {
  function handleClick() {
